...
if (!require("readxl")) install.packages("readxl")# install the readxl package
...
single_gds <- get("single_gds")#Method 3
GS_non_na<- na.omit(single_gds)#Delete all subjects with missing values



#####1.4 Construct an Ising network and perform network analysis######
gs_fit<-estimateNetwork(GS_non_na,default = "IsingFit",rule="AND")#Fit Ising model
