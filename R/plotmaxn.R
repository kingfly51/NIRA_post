...
plotmaxn <- function(maxn,low=1,high=3) {
  percent_cols <- grep("percenttop", colnames(maxn), value = TRUE)
  args_list <- lapply(maxn[, percent_cols], function(x) -x)
  args_list <- c(args_list, list(decreasing = FALSE))
  sorted_idx <- do.call(order, args_list)
  node_order <- rownames(maxn)[sorted_idx]
  maxn_ordered <- maxn[node_order, ]

  maxn_wide<-maxn_ordered[,low:high]
