...
permutation_test <- function(original, group, n_perm = 5000) {
  ...
  s_pooled <- as.numeric(sqrt(((n1 - 1) * var_original + (n2 - 1) * var_group) / (n1 + n2 - 2)))
  ...
  for (i in 1:n_perm) {
    labels <- sample(rep(c(1, 2), times = c(n1, n2)))
    new_original <- combined[labels == 1]
    new_group <- combined[labels == 2]
